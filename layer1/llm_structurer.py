...
llm_structurer.py
-----------------
PURPOSE: Sends exam paper images DIRECTLY to Gemini Vision and gets
         structured JSON back in one single API call.

NEW APPROACH (replacing EasyOCR pipeline):
  Before: PDF → images → EasyOCR (garbled text) → Gemini (tries to fix garbage)
  Now:    PDF → images → Gemini Vision (reads handwriting + structures it) ✓

WHY THIS IS BETTER:
  - Gemini is a multimodal model — it can SEE images natively, just like a human.
  - EasyOCR was a pure character recognizer with no context awareness.
    It output jumbled characters that Gemini could not meaningfully fix.
  - Gemini reads the actual handwriting, understands spatial layout (indentation,
    question numbering, sub-parts), and returns clean structured JSON.

WHY WE SEND ALL PAGES IN ONE CALL:
  If Q1 part (a) is on page 2 and part (b) is on page 3, a per-page approach
  would split the answer. One combined call lets Gemini see the full exam and
  correctly group continuations of the same question across page breaks.

COST:
  Gemini 2.5 Flash free tier: 500 requests/day.
  One exam paper (even 10 pages) = 1 API call. Easily free for a hackathon.
"""
...
# ---- The Prompt ----
# This prompt is sent along with all the page images.
# Gemini receives both the images AND this text at the same time
# and uses both together to produce the output.
#
STRUCTURING_PROMPT = """\
