...
from pdf_converter import pdf_to_images, load_image
from llm_structurer import structure_images   # <-- No more EasyOCR import
...
def process_file(file_path: str) -> dict:
    """
    Full pipeline for a single exam paper (image or PDF).

    New pipeline flow:
        PDF   →  [pdf_converter]  →  list of PIL Images
        Image →  [load directly]  →  single PIL Image in a list
        Images → [llm_structurer] →  Gemini Vision → structured JSON dict

    Args:
        file_path (str): Path to an image file or PDF

    Returns:
        dict: Full result including metadata + structured answers
    """
